...
public class Counts implements Serializable {
	...
	public Counts(CountContext context) {
	...
	public boolean isProtocolCountsNew() {
		if (protocolCounts == null || protocolCounts.size() == 0) {
			return false;
		}
		for (ProtocolCount protocolCount : protocolCounts) {
	...
	private int getActiveIndex(List<FinalCount> counts) {
		...
		for (FinalCount count : counts) {
			if (count.isApproved()) {
				...
				approved = true;
			} else if (!approved && (lastModifiedDate == null || lastModifiedDate.before(count.getModifiedDate()))) {
				result = count.getIndex() - 1;
	...
	public boolean hasApprovedPreliminaryCount() {
		if (!hasPreliminaryCount()) {
			return false;
		}
		return preliminaryCount.isApproved();
	}
	...
	public boolean hasApprovedProtocolAndPreliminaryCount() {
		if (!hasProtocolAndPreliminaryCount()) {
			return false;
		}
		return protocolAndPreliminaryCount.isApproved();
	}
