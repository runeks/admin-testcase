...
@NamedQueries({
		...
		@NamedQuery(name = "LocaleText.findByElectionEventAndTextId", query = "select lt from LocaleText lt join lt.textId ti "
				+ "where ti.electionEvent.pk = :electionEventPk and lt.locale.pk = :localePk"),
		@NamedQuery(name = "LocaleText.findByElectionEventLocaleAndTextId", query = "select lt from LocaleText lt inner join lt.textId ti where"
				+ " ti.electionEvent.pk = :electionEventPk and lt.locale.pk = :localePk and ti.textId = :textId"),
		@NamedQuery(name = "LocaleText.findTextsByElectionEventLocaleAndTextId", query = "select ti.textId, lt.localeText from"
				+ " LocaleText lt inner join lt.textId ti where"
				+ " ti.electionEvent.pk = :electionEventPk and lt.locale.pk = :localePk and ti.textId IN (:textIds)"),
		...
		@NamedQuery(name = "LocaleText.findByTextIdPkAndLocalePk", query = "select l from LocaleText l"
				+ " WHERE l.textId.pk = :textIdPk AND l.locale.pk = :localePk"),
...
public class LocaleText extends VersionedEntity implements java.io.Serializable {
	...
	@JoinColumn(name = "text_id_pk", nullable = false)
	...
	public String getAuditLogId() {
		return AuditLogUtil.generateId(textId, locale);
	}
}
