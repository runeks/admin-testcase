...
@NamedQueries({
		@NamedQuery(name = "ContestVoting.countElectoralRollCheckCatDate", query = "SELECT COUNT(cv) FROM ContestVoting cv where cv.id.contestPk = :conpk AND "
				+ "cv.votingCategoryId = :cat AND cv.pollingDistrictPk = :pdpk AND cv.castTimestamp BETWEEN :dateStart AND :dateEnd"),
		@NamedQuery(
				name = "ContestVoting.countElectoralRollCheckCatDateVoting",
				query = "SELECT COUNT(cv) FROM ContestVoting cv where cv.id.contestPk = :conpk AND "
						+ "cv.votingCategoryId = :cat AND cv.votingPollingDistrictPk = :pdpk AND " + "cv.castTimestamp BETWEEN :dateStart AND :dateEnd"),
		@NamedQuery(
...
public class ContestVoting implements java.io.Serializable {
	...
	@Column(name = "electronic_voting")
	public Boolean getElectronicVoting() {
